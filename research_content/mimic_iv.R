...
d_d <- read_excel("./var_dict_mimic.xlsx", sheet=2, col_names=T)
...
get_data <- function(k) {
    ...
    uo_k <- urine_output[which(urine_output$stay_id == cur_stid), ,drop=F]
...
d_all <- do.call(rbind, list(d_vit, d_bg, d_gcs, d_uo, d_vaso, d_vent, d_cbc, d_chem))
as.data.frame(d_all)
write.csv(d_all, file="./d_all_mimic.csv", row.names=F, fileEncoding="gb2312", na="")
