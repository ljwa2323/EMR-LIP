...
d_d <- read_excel("./var_dict_eicu_crd.xlsx", sheet=2, col_names=T)
...
get_data <- function(k) {
    ...
    o2_k1 <- resample_point_wide(o2_k,
                                d_o2$item_id,
                                d_o2$value_type,
                                d_o2$agg_fun, 
                                seq(t0, t2-t1, 1),
    ...
    lab_k1 <- resample_point_wide(lab_k,
                                d_lab$item_id,
                                d_lab$value_type,
                                d_lab$agg_fun, 
                                seq(t0, t2-t1, 1),
...
d_all <- do.call(rbind, list(d_bg, d_vit, d_gcs, d_uo, d_med, d_o2, d_lab))
write.csv(d_all, file="./d_all_eicu.csv", row.names=F, fileEncoding="gb2312", na="")
