...
from dataloader import EICU_CRD
from model import *
...
pre_param = None
file_out = f"/home/luojiawei/EMR_LIP/结果文件夹/模型性能_share/{model_name}_eicu_{y_name}_dym.csv"
# -------  模型参数 -----------
...
loss_fn = nn.BCEWithLogitsLoss(pos_weight = torch.tensor([0.9816 / (1-0.9816)]).to(device))
# loss_fn = nn.CrossEntropyLoss(weight=torch.tensor([0.0655,0.5027,0.4318]).to(device))

print(f"task: {y_name}")


dataset_tr = EICU_CRD("/home/luojiawei/EMR_LIP_data/eicu_crd/all_stids_share/",
                "/home/luojiawei/EMR_LIP_data/ds_id_eicu_crd.csv",
...
dataset_va = EICU_CRD("/home/luojiawei/EMR_LIP_data/eicu_crd/all_stids_share/",
                "/home/luojiawei/EMR_LIP_data/ds_id_eicu_crd.csv",
                id_col="patientunitstayid",
                mod_col="set",
                mod = [3],
    ...
    with torch.no_grad():
        ...
        try:
            ...
            if auc > best_auc:
                ...
                torch.save(model.state_dict(), f"/home/luojiawei/EMR_LIP/deep_learning/saved_param_share/{model_name}_eicu_{y_name}_dym" + ".pth")
                no_improvement_count = 0
                print("参数已更新")
            else:
            ...
            torch.save(model.state_dict(), f"/home/luojiawei/EMR_LIP/deep_learning/saved_param_share/{model_name}_eicu_{y_name}_1_dym" + ".pth")
